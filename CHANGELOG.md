...
#### 11. CLI メッセージ出力の英語化(Issue #8)

cfnsync が組み立てる進捗・エラー・警告・承認プロンプト・ヘルプ・report のテキスト表現をすべて英語へ置き換えました(要件は [docs/spec/requirements.md](./docs/spec/requirements.md) の NFR-7)。

- **変更前**: 上記のメッセージの多くが日本語の固定文言でした。
- **変更後**: すべて英語の固定文言になります。利用者の設定・テンプレート・AWS からの応答に含まれる値(スタック名、パス、`ResourceStatusReason` 等)をそのまま提示する箇所はこの対象外で、値自体の文字種は変わりません。`--output json` の固定列挙値(`error.type` の値、`ReconciliationRecord.kind` の値等)も対象です。JSON のフィールド名・構造・終了コードは変わりません。

**移行**: 標準出力・標準エラー出力の特定の日本語文言を `grep` 等で検出しているスクリプトがあれば、対応する英語文言に置き換えてください。機械可読な判定には引き続き終了コードと JSON のフィールド(`error.type`、`outcome` 等)を使うことを推奨します。本書・README・spec ドキュメント・ソースコードのコメントは対象外で、引き続き日本語のままです。
