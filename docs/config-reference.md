...
| `tags` | `Record<string, string \| number \| boolean>` | 任意 | `{}` | スタックタグ。値の扱いは `parameters` と同様 |
...
| `regionOverrides` | `Record<region, { parameters, tags }>` | 任意 | `{}` | 対象リージョンの `parameters`/`tags` を共通値へ**浅くマージ**する(同名キーのみ上書き、他のキーは共通値を維持)。`capabilities`/`dependsOn`/`stackName`/`regions` はリージョン単位で上書きできない |
