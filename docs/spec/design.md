...
- **敵対的レビュー指摘の是正(FR-6-9a)**: 削除待ちの識別子は `(リージョン, スタック名)` だけで決まる。無関係な過去のリネームや過去実行の積み残しが同じスタック名で別の削除待ちを残している場合、「当該 ID の削除待ちが存在する」というだけでは「今回の対の create が成功した」ことの証明にならない。実装は削除待ちレコードの `originStackKey` が、削除しようとしている旧スタックの対となるスタックキー(今回の `added` 側のスタックキー)と一致することまで確認する。一致しない場合は、たとえ同じ ID の削除待ちが存在していても削除を拒否する。
- **敵対的レビュー指摘の是正・2回目(FR-6-9a)**: `originStackKey` の一致だけでは、同一テンプレートキーが過去に同じスタック名でリネームして削除待ちを残し、その後 config が旧名へ戻され(`import` 等で)再び同じキーの下で管理された場合を判別できない。この場合、今回のリネームの対も同じテンプレートキーを使うため `originStackKey` が偶然一致してしまう。実装はさらに、`state.stacks[operation.stackKey]` が現在保持する `stackId` が、削除しようとしている旧スタックの `stackId` と異なることまで確認する。今回の対の create が失敗・未実行であれば、このキーは削除対象と同じ `stackId` のままであり、一致する限り削除を拒否する(fail-closed)。リネーム対の `added` 側は常に旧名側と同一のスタックキーを持つ(FR-1-14)ため、このキーの `stackId` が変わりうるのは、今回または過去の実行での**この対自身の** create が成功した場合に限られる — 無関係な他のスタックがこのキーへ書き込まれることはない。したがって stackId の不一致は「対の create 成功」の十分な証拠であり、成功が今回の実行内か過去の実行かは問わない(過去の実行で create のみ成功し削除が未実行だった場合の収束は、Issue #16 の意図どおり許可する)。
