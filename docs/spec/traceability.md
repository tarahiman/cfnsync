...
| FR-6-7〜FR-6-13 削除待ちの削除 | §5.2, §5.5, §8.3 | `usecase/deploy`(統合グラフ・物理衝突判定), `usecase/delete`(`originStackKey` 突き合わせ・物理スタック二重所有チェック) | `test/usecase/pendingDeletion.test.ts` (`FR-6-7`〜`FR-6-9a`, `FR-6-11`), `test/usecase/delete.test.ts` (`FR-6-8`, `FR-6-13`), `test/usecase/status-graph.test.ts` (`FR-6-12`) |
