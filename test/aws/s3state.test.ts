...
import { createInitialState, serializeState, type CfnSyncState } from '../../src/core/state.js';
...
function makeBackend(): S3StateBackend {
  return new S3StateBackend({ bucket: BUCKET, key: KEY, region: 'ap-northeast-1' });
}
...
const LOCK_INFO: LockInfo = { runId: 'run-1', startedAt: '2026-07-19T00:00:00Z', owner: 'ci@github' };
...
describe('S3StateBackend load/save (FR-1-6 s3)', () => {
  it('load: NoSuchKey は undefined、それ以外は ETag を version に保持する', async () => {
    ...
    s3Mock
      ...
      .resolves({ Body: bodyOf(serializeState(stateWith(4))), ETag: '"etag-4"' } as never);
  ...
  it('FR-1-6(s3): 既存版の保存は PutObject If-Match: <読込時 ETag> で行い、新 ETag を返す', async () => {
    ...
    const version = await makeBackend().save(stateWith(5), { generation: 4, etag: '"etag-4"' });
...
describe('S3StateBackend lock (FR-1-7 / FR-1-8 / FR-1-9 / FR-1-10)', () => {
  it('FR-1-7 / FR-1-10: acquireLock は <key>.lock を If-None-Match: * で作成し、内容に runId/startedAt/owner を書く', async () => {
    s3Mock.on(PutObjectCommand, { Key: LOCK_KEY }).resolves({ ETag: '"lock-1"' });
  ...
  it('FR-1-7: 既存ロックあり(PreconditionFailed)→ LockError、他の書き込みは発生しない', async () => {
    s3Mock.on(PutObjectCommand, { Key: LOCK_KEY }).rejects(preconditionFailed());
    await expect(makeBackend().acquireLock(LOCK_INFO)).rejects.toBeInstanceOf(LockError);
  ...
  it('FR-1-8: releaseLock は DeleteObject If-Match: <取得時 ETag> で解放する', async () => {
    ...
    const result = await makeBackend().releaseLock({ runId: 'run-1', etag: '"lock-1"' });
  ...
  it('FR-1-8: 条件不成立(所有者交代・412)→ 削除せず released:false を報告する', async () => {
    s3Mock.on(DeleteObjectCommand, { Key: LOCK_KEY }).rejects(preconditionFailed());
    const result = await makeBackend().releaseLock({ runId: 'run-1', etag: '"lock-1"' });
  ...
  it('FR-1-7(冪等): releaseLock を 2 回呼んでもエラーにならない', async () => {
    ...
    const first = await backend.releaseLock({ runId: 'run-1', etag: '"lock-1"' });
    const second = await backend.releaseLock({ runId: 'run-1', etag: '"lock-1"' });
  ...
  it('FR-1-10: readLock はロックオブジェクトの内容を返す(未ロックは undefined)', async () => {
    ...
    s3Mock.reset();
    s3Mock
      .on(GetObjectCommand, { Key: LOCK_KEY })
      .resolves({ Body: bodyOf(JSON.stringify(LOCK_INFO)), ETag: '"lock-1"' } as never);
  ...
  it('FR-1-9(基盤): verifyLock は runId・ETag が一致すれば true、不一致・消失は false', async () => {
    ...
    s3Mock
      .on(GetObjectCommand, { Key: LOCK_KEY })
      .resolves({ Body: bodyOf(JSON.stringify({ ...LOCK_INFO, runId: 'run-2' })), ETag: '"lock-1"' } as never);
    ...
    s3Mock
      .on(GetObjectCommand, { Key: LOCK_KEY })
      .resolves({ Body: bodyOf(JSON.stringify(LOCK_INFO)), ETag: '"lock-2"' } as never);
    expect(await makeBackend().verifyLock(handle)).toBe(false);
...
describe('S3StateBackend stateId', () => {
  it('bucket/key から安定な短縮ハッシュを導出する', () => {
    const a = new S3StateBackend({ bucket: BUCKET, key: KEY, region: 'ap-northeast-1' }).stateId();
    const b = new S3StateBackend({ bucket: BUCKET, key: KEY, region: 'us-east-1' }).stateId();
    const c = new S3StateBackend({ bucket: BUCKET, key: 'other.json', region: 'ap-northeast-1' }).stateId();
