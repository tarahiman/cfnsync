...
**Further fix from a follow-up review (`FR-6-9a`)**: the `originStackKey` correlation alone still missed a case where the *same* template key had, in an earlier run, renamed the same stack name and left a pending deletion, and config was later changed back to the old name (via `import` or similar) under that same key. Since the template key itself never changed, `originStackKey` happens to match this run's own rename pair too, so the old stack could still be deleted even when the new stack's creation failed. The check now also requires that the `stackId` currently held by that stack key differs from the `stackId` being deleted (i.e., that key has genuinely been replaced by a different physical stack).
