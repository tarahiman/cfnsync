...
`state` を省略した場合は `local` バックエンドとなり、設定ファイルと同じディレクトリに `cfnsync.state.json` を保存します。`local` は単一環境専用です。複数の実行環境（CI ランナー等）から利用する場合は、ロックと条件付き書き込みを備えた `s3` バックエンドを使用してください。S3 バケットはバージョニングの有効化を推奨します。
...
変更セット名だけでは敵対的な同一アカウント主体による所有権偽装を原理的に防げないため、IAM で cfnsync の操作主体を分離してください。
