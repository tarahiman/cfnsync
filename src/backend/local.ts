 ...
 *
 * design.md §4.5(バックエンドと排他制御)に対応する。`local` の save は同階層の
 * `<state>.lock` を O_EXCL で取得し、世代比較から rename までを排他する(FR-1-6 local)。
...
export class LocalStateBackend implements StateBackend {
  ...
  async save(
    ...
    const saveLockPath = `${this.statePath}.lock`;
    let saveLock: Awaited<ReturnType<typeof open>>;
    try {
    ...
    try {
      // FR-1-6(local): mutex 保持中に再読込して世代を比較し、rename まで同じ排他区間に置く。
